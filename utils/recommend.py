...
def maximum_matching(train_text, # 上班部分
                     decode_text, # 下半部分
                     entityRe=r'\[\@.*?\#.*?\*\](?!\#)',
                     recommendRe=r'\[\$.*?\#.*?\*\](?!\#)'):
    # print "Training data:"
    ...
    for match in re.finditer(entityRe, decode_no_recommend):
        ...
        [entity, recognized_type] = recommend_entity.strip('[@]*').rsplit(
            '#', 1)
        decode_origin += entity
        entity_recognized_list += [
            "B-@-" + recognized_type
        ] + ["I-@-" + recognized_type] * (len(entity) - 1)
        last_entity_end = match.span()[1]
    decode_origin += decode_no_recommend[last_entity_end:]
    entity_recognized_list += [0] * (
        len(decode_no_recommend) - last_entity_end)
    ...
    while FMM_start < origin_length:
        ...
        if FMM_end == FMM_start:
            ...
            FMM_end = (FMM_start + max_length) if (
                FMM_start +
                max_length) < origin_length - 1 else origin_length - 1
        ...
        ## finish recognized span detection
            ...
            if word in extracted_dict:
                entity_recommend_list += [
                    "B-$-" + extracted_dict[word]
                ] + ["I-$-" + extracted_dict[word]] * (FMM_end - FMM_start - 1)
                FMM_start = FMM_end
                FMM_end = (FMM_start + max_length) if (
                    FMM_start +
                    max_length) < origin_length - 1 else origin_length - 1
    ...
    recommend_decode_text = merge_text_with_entity(
        decode_origin, entity_recognized_list, entity_recommend_list)
